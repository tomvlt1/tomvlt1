## Hey, I'm Tom.

IE Business School, London. Dual Business + Data Analytics. I build things that sit between finance and software. Currently stealth; what's public is below.

### Projects

- **[madrid-rental-vision](https://github.com/tomvlt1/madrid-rental-vision)** — can a photo tell you how much a flat rents for? About +0.05 R² says yes. Ships with a Chrome extension and a Next.js dashboard on top of the model.
- **[bullseye](https://github.com/tomvlt1/bullseye)** — Chrome extension that fact-checks highlighted text with a confidence score and supporting / contradicting sources. Manifest V3.
- **[speaker-verification](https://github.com/tomvlt1/speaker-verification)** — I wanted a way to grade class participation from the actual audio of the class, not from professor memory. Wraps Azure Speaker Recognition.
- **[Lock-in](https://github.com/tomvlt1/Lock-in)** — iOS habit tracker I built to keep myself accountable. Sideload via Sideloadly.

### Stack

Python (PyTorch, scikit-learn, FastAPI) · Swift (SwiftUI, SwiftData) · TypeScript (React, Next.js, React Native) · Supabase · a bit of everything.

### Reach

- [LinkedIn](https://www.linkedin.com/in/thomas-valentin11/)
- tom.valentinuk@gmail.com
